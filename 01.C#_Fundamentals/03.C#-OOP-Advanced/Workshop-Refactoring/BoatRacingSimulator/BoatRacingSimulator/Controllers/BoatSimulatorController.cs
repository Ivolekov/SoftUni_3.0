namespace BoatRacingSimulator.Controllers
{
    ...
    public class BoatSimulatorController : IBoatSimulatorController
    {
        ...
        public string StartRace()
        {
            ...
            var orderedBoats = participants.Where(x => x.CalculateRaceSpeed(this.CurrentRace) > 0)
            ...
            for (int i = 0; i < orderedBoats.Count; i++)
            {
                var speed = orderedBoats[i].CalculateRaceSpeed(this.CurrentRace);
                var time = this.CurrentRace.Distance / speed;

                result.AppendLine(string.Format(
                "{0} place: {1} Model: {2} Time: {3}",
                (Positions)i,
                orderedBoats[i].GetType().Name,
                orderedBoats[i].Model,
        ...
        {
            var participants = this.CurrentRace.GetParticipants();
            double a = 100 / participants.Count;
            Dictionary<string, double> dict = new Dictionary<string, double>();
            foreach (var participant in participants)
            {
                string name = participant.ToString().Replace("BoatRacingSimulator.Models.Boats.", string.Empty);
            ...
            {
                sb.AppendLine($"{d.Key} -> {d.Value:F2}%");
            }
